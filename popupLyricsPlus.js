...
function PopupLyrics() {
    ...
    async fetchLrclib(info) {
      ...
      const lyrics = lines.map((line) => {
        ...
        if (line.trim() !== "" && isSynced && time) {
          return {
            text: lyric || "♪",
            startTime: Number(min) * 60 + Number(sec),
          };
        }
        return;
      });
  ...
  const userConfigs = {
    ...
    translation2rd:
      Number(LocalStorage.get("popup-lyrics:translation-2rd")) || -1,
    showOriginal: LocalStorage.get("popup-lyrics:show-original") || "0", // 0: trans only, 1: original only, 2: both
  ...
  // Register Media Session action handlers so PiP shows play/pause/prev/next controls
  ...
  async function updateTrack() {
    ...
    for (const name of userConfigs.servicesOrder) {
      ...
      try {
        ...
        // Auto-detect language if translation2rd is -1 (Auto)
        let activeTranslation2rd = userConfigs.translation2rd;
        if (activeTranslation2rd === -1 || activeTranslation2rd === "-1") {
          const allText = data.lyrics.map((lyric) => lyric.text).join(" ");
          activeTranslation2rd = detectLanguage(allText);
          // console.log("Auto-detected translation mode:", activeTranslation2rd);
        }

        // Store original text before translation
        if (activeTranslation2rd !== 0) {
          for (let i = 0; i < data.lyrics.length; i++) {
            if (!data.lyrics[i].original) {
              data.lyrics[i].original = data.lyrics[i].text;
            }
          }
        }

        // Translate into Eng/Romaji
        if (activeTranslation2rd == 3) {
          //Romaji
        ...
        } else if (activeTranslation2rd == 1) {
          //English
        ...
        } else if (activeTranslation2rd == 4) {
          //Pinyin
        ...
        } else if (activeTranslation2rd == 5) {
          //Romaja
  ...
  function detectLanguage(text) {
    ...
    // Return translation2rd based on detected language
    // 3: Romaji (Japanese), 5: Romanja (Korean), 4: Pinyin (Chinese)
    if (japanese > korean && japanese > chinese) {
      return 3; // Romaji for Japanese
    } else if (korean > japanese && korean > chinese) {
      return 5; // Romanja for Korean
    } else if (chinese > japanese && chinese > korean) {
      return 4; // Pinyin for Chinese
    }

    return 0; // No translation if no special characters detected
  }
  ...
  // Function to draw both original and translation with synchronized animation
  function drawParagraphWithSplit(ctx, displayData, options) {
    const { original, translation } = displayData;
    const showTranslation = options.showTranslation !== false; // default true

    if (!translation || !showTranslation) {
      // If no translation or showTranslation is false, just draw the original normally
      return drawParagraph(ctx, original, options);
    }
    ...
    // MEASUREMENT PHASE: Get dimensions without renderring yet
    // Get original dimensions
    const origMeasureResult = drawParagraph(ctx, original, {
      ...options,
      measure: true,
      translateY: 0, // Don't apply translateY during measurement
    });

    // Setup translation parameters
    const translationFontSize = Math.floor(options.fontSize * 0.65);
    const translationLineHeight = translationFontSize * 1.2;
    const gap = options.fontSize * 0.3;
    const fontFamily = options.fontFamily;

    // Get translation dimensions
    ctx.font = `bold ${translationFontSize}px ${fontFamily}`;
    const transMeasureResult = drawParagraph(ctx, translation, {
      ...options,
      top: origMeasureResult.bottom + gap,
      bottom: undefined,
      vCenter: false,
    ...
    const totalHeight =
      origMeasureResult.height + transMeasureResult.height + gap;

    // Calculate translateY based on TOTAL HEIGHT (not just original)
    let translateY = 0;
    if (typeof options.translateY === "function") {
      translateY = options.translateY(totalHeight);
    } else if (typeof options.translateY === "number") {
      translateY = options.translateY;
    }

    // RENDERING PHASE: Actually draw with correct synchronized translateY
    // Restore original font before rendering
    ctx.font = `bold ${options.fontSize}px ${fontFamily}`;

    // Draw original with the calculated translateY
    const origResult = drawParagraph(ctx, original, {
      ...options,
      translateY, // Apply the synchronized translateY
      measure: false,
    });

    // Draw translation positioned relative to translated original
    ctx.font = `bold ${translationFontSize}px ${fontFamily}`;
    const transResult = drawParagraph(ctx, translation, {
      ...options,
      top: origResult.bottom + gap, // Positioned after translated original
      bottom: undefined,
      vCenter: false,
      lineHeight: translationLineHeight,
      fontSize: translationFontSize,
      translateY: 0, // No additional translateY (positioning already handled)
      measure: false,
    });
    ...
    return {
      width: Math.max(origResult.width, transResult.width),
      height: totalHeight,
      left: origResult.left,
      right: Math.min(origResult.right, transResult.right),
      top: origResult.top,
      bottom: transResult.bottom,
    };
  }
  ...
  function renderLyrics(ctx, lyrics, currentTime) {
    // console.log("Rendering lyrics at time:", currentTime);
    const focusLineFontSize = userConfigs.fontSize;
    const focusLineHeight = focusLineFontSize * 1.2;
    const prevLineMargin = focusLineFontSize * 2;
    const nextLineMargin = focusLineFontSize * 1;
    const otherLineFontSize = focusLineFontSize * 0.85;
    ...
    const showOriginal = userConfigs.showOriginal;

    // Helper function to get display text
    function getDisplayText(lyric) {
      if (showOriginal === "1") {
        // Show original only
        return { original: lyric.original || lyric.text, translation: null };
      } else if (showOriginal === "2") {
        // Show both: original on top, translation below
        if (lyric.original && lyric.text.trim() !== lyric.original.trim()) {
          return { original: lyric.original, translation: lyric.text };
        }
        return { original: lyric.text, translation: null };
      }
      // Default: show translation/current text only
      return { original: lyric.text, translation: null };
    }
    ...
    const prevLineFocusHeight = drawParagraphWithSplit(
      offscreenCtx,
      lyrics[currentIndex - 1]
        ? getDisplayText(lyrics[currentIndex - 1])
        : { original: "", translation: null },
      {
        ...
        fontSize: focusLineFontSize,
        fontFamily: fontFamily,
        showTranslation: false,
        measure: true,
      },
    ).height;
    const pos = drawParagraphWithSplit(
      offscreenCtx,
      getDisplayText(lyrics[currentIndex]),
      {
        vCenter: true,
        hCenter,
        left: marginWidth,
        ...
        translateY: (selfHeight) =>
          ((prevLineFocusHeight + selfHeight) / 2 + prevLineMargin) *
          (1 - progress),
      },
    );
    ...
    for (let i = 0; i < currentIndex && i < 1; i++) {
      if (i === 0) {
      ...
      lastBeforePos = drawParagraphWithSplit(
        offscreenCtx,
        getDisplayText(lyrics[currentIndex - 1 - i]),
        {
          hCenter,
          bottom:
            i === 0
              ? lastBeforePos.top - prevLineMargin
    ...
    for (let i = currentIndex + 1; i < lyrics.length; i++) {
      // console.log("Drawing line:", lyrics[i].text);
      lastAfterPos = drawParagraphWithSplit(
        offscreenCtx,
        getDisplayText(lyrics[i]),
        {
          hCenter,
          top:
            i === currentIndex + 1
              ? lastAfterPos.bottom + nextLineMargin
              : lastAfterPos.bottom + otherLineMargin,
          left: marginWidth,
          right: otherRight,
          lineHeight: otherLineHeight,
          fontSize: otherLineFontSize,
          fontFamily: fontFamily,
          showTranslation: true,
        },
      );
  ...
  function scheduleNextTick(options) {
    cancelScheduled();

    rafId = requestAnimationFrame(() => tick(options));
    // if (!document.hidden) {
    // rafId = requestAnimationFrame(() => tick(options));
    // } else {
    //   timeoutId = setTimeout(() => tick(options), 20); // cukup 4fps
    // }
  }
  ...
  async function tick(options) {
    ...
    if (error) {
      ...
      drawText(lyricCtx, "Loading");
    }

    // kalau belum ada lyrics, jangan 60fps
    if (!lyrics?.length) {
      timeoutId = setTimeout(() => tick(options), 1000);
      return;
    }

    scheduleNextTick(options);
  }
...
`;
      ...
      const translation2 = createOptions(
        ...
        {
          "-1": "Auto (JP: Romaji, CN: Pinyin, KR: Romanja)",
          0: "None",
          1: "English (Google)",
          2: "Chinese (NeteaseCN)",
      ...
      const showOriginalOption = createOptions(
        "Show original lyrics",
        {
          0: "Translation only",
          1: "Original only",
          2: "Both (Original + Translation)",
        },
        userConfigs.showOriginal,
        (state) => {
          userConfigs.showOriginal = state;
          LocalStorage.set("popup-lyrics:show-original", state);
        },
      );
